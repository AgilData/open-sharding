    ...
    public int neword(int t_num,
        ...
        try {
            ...
            try {
                ...
                if (TRACE)
                    logger.trace("TOOK " + (t2 - t1) + " ms to run: SELECT c_discount, c_last, c_credit, w_tax FROM customer, warehouse WHERE w_id = " + w_id + " AND c_w_id = " + w_id + " AND c_d_id = " + d_id + " AND c_id = " + c_id);
