...
public class Delivery implements TpccConstants {
	...
	public int delivery( int t_num, int w_id_arg, int o_carrier_id_arg, Connection conn, TpccStatements pStmts){
		int w_id = w_id_arg;
		...
		for (d_id = 1; d_id <= DIST_PER_WARE; d_id++){
			...
			try {
				...
				pStmts.getStatement(28).executeQuery();
			} catch (SQLException e) {
			...
			try {
				...
				pStmts.getStatement(29).executeQuery();
			} catch (SQLException e) {
			...
			try {
				...
				pStmts.getStatement(31).executeQuery();
			} catch (SQLException e) {
				throw new RuntimeException("Delivery Update transaction error", e);
			}
