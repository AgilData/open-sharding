...
def create_tar(mysql_version)
    ...
    if mysql_version.match("5.0")
        run_command "cp ../mysql-install-5.0/libmysql_r/.libs/libmysqlclient_real.so.15.0.0 _temp"
    elsif mysql_version.match("5.1")
        run_command "cp ../mysql-install-5.1/libmysql_r/.libs/libmysqlclient_real.so.16.0.0 _temp"
    else
